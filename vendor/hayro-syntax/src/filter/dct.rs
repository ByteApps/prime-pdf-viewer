// LOCAL PATCH (prime-pdf-viewer, vendored hayro-syntax 0.7.2, 2026-08-17):
// `decode()` now tries a scaled decode via `jpeg-decoder`'s IDCT downscaling
// (1/8, 1/4, 1/2, 1/1) when `ImageDecodeParams::target_dimension` hints that
...
///
/// `jpeg-decoder` requires `std` (it decodes from `std::io::Read`
/// unconditionally); this vendored copy is always built with the default
/// `std`+`images` features, so that's satisfied here.
///
...
fn try_scaled_decode(
    ...
    // jpeg-decoder's scale() takes u16 dimensions; bail (rather than
    // truncate) if the hint is somehow outside that range.
    ...
    // jpeg-decoder doesn't apply a PDF /DecodeParms ColorTransform override;
    // skip when one is present rather than risk diverging from zune-jpeg's
    // color handling.
