    ...
    fn select_worker(frame: &FrameInfo, worker_preference: PreferWorkerKind) -> PreferWorkerKind {
        ...
        match worker_preference {
            ...
            PreferWorkerKind::Multithreaded => {
                // VENDORED PATCH (ByteApps, 2026-08-19): never pick the mpsc
                // worker on KeyOS. Its channels are UNBOUNDED and receive one
                // Vec<i16> per component per MCU row; on a single-core device
                // the decode loop floods them faster than the worker threads
                // are scheduled, so the queues grow toward the whole image's
                // coefficients (~3 bytes/pixel for 4:2:0 -- 108 MB for a
                // 6000x6000 photo) and exhaust the Passport Prime's 128 MB
                // physical pool: the kernel kills the app with an
                // uncatchable "Out of physical memory" at first touch.
                // ImmediateWorker does the IDCT inline in bounded memory.
                #[cfg(target_os = "xous")]
                {
